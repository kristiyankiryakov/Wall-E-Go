...
type TransactionRepository interface {
	...
	UpdateStatusBatch(ctx context.Context, transactionIDs []string, status TransactionStatus) error
	UpdateStatusConcurrently(ctx context.Context, transactionIDs []string, status TransactionStatus) error
}
...
func (r *PostgresTransactionRepository) UpdateStatusBatch(ctx context.Context, transactionIDs []string, status TransactionStatus) error {
	...
	// Execute update
	_, err = tx.ExecContext(ctx, query, string(status), time.Now(), transactionIDs, string(StatusPending))
	if err != nil {
...
func (r *PostgresTransactionRepository) UpdateStatusConcurrently(ctx context.Context, transactionIDs []string, status TransactionStatus) error {
	if len(transactionIDs) == 0 {
		return nil
	}
